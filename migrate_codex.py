...
def replace_skill_plain_inline(text: str) -> str:
    # Leftover non-backticked inline examples. Bare full-line calls inside code
    # fences are handled above; these catch prose such as "invoke Skill(...)".
    text = re.sub(
        r'Skill\(skill="([^"]+)",\s*args="([^"]*)"\)',
        r'Invoke /\1 \2',
        text,
...
def replace_paths(text: str) -> str:
    text = re.sub(r'~/\.claude/lessons(?=/|\b)', '~/.codex/lessons', text)
    text = re.sub(r'~/\.claude/skills(?=/|\b)', '~/.codex/skills', text)
    text = re.sub(r'~/\.Codex/', '~/.codex/', text)
    return text
...
def replace_removed_poc_routing(text: str) -> str:
    text = text.replace(
        '**`Bash("curl ...")` with `poc=True`** — only set this flag when the request is a confirmed, report-worthy exploit. Routes through Burp Suite for HTTP History capture. Do NOT use for recon probes.',
        '**Confirmed HTTP exploits** — write a `pocs/<title>.http` file with the minimal raw request. Paste it into Burp Repeater manually when needed.',
    )
    text = text.replace(
        'Burp routing via `poc=True`',
        'Burp routing via a special PoC flag',
    )
    return text
...
def transform_md(text: str) -> str:
    ...
    text = replace_skill_plain_inline(text)
    ...
    text = replace_removed_poc_routing(text)
...
def transform_text_artifact(text: str) -> str:
    text = replace_paths(text)
    text = replace_claude_md_refs(text)
    text = replace_removed_poc_routing(text)
    return text
...
> **Codex CLI compatibility.** This file is the Codex equivalent of `CLAUDE.md`
> in the source repo. Codex discovers skills under
...
>
> **Source of truth.** This file is generated. Do not edit by hand — edit `CLAUDE.md`
> in the source repo, then re-run `migrate_codex.py`.
...
SKIP_FILES = {
    ...
    "migrate_api.py",     # upstream migration utility, not part of Codex mirror
    "migrate_codex.py",   # this script
    "migrate_native.py",  # source-tree-only utility
}

SKIP_DIRS = {".git", ".github", ".claude", ".vscode", "__pycache__"}
...
def write_file(src_path: Path, out_dir: Path, src_root: Path, kind: str) -> bool:
    ...
    if kind == 'md':
        ...
        new_text = transform_text_artifact(transform_py(src_path.read_text(encoding='utf-8')))
        write_bytes = new_text.encode('utf-8')
    elif src_path.suffix in {'.txt', '.log', '.json', '.jsonl'}:
        new_text = transform_text_artifact(src_path.read_text(encoding='utf-8'))
        write_bytes = new_text.encode('utf-8')
    else:
...
FORBIDDEN_PATTERNS = [
    ...
    (re.compile(r'~/\.claude/'), 'unrewritten ~/.claude/ path'),
    (re.compile(r'~/\.Codex/'), 'incorrect ~/.Codex path casing; use ~/.codex'),
    (re.compile(r'poc=True'), 'removed Burp poc=True routing reference'),
    (re.compile(r'docs\.anthropic\.com/en/docs/(?:Codex|claude-code)', re.IGNORECASE),
     'stale Anthropic/Codex install URL'),
...
def iter_verifiable_files(out_dir: Path):
    for suffix in ("*.md", "*.py"):
        yield from sorted(out_dir.rglob(suffix))
...
def verify_output(out_dir: Path) -> int:
    ...
    for p in iter_verifiable_files(out_dir):
        ...
        for i, line in enumerate(text.splitlines(), 1):
            ...
            if re.search(r'\bCLAUDE\.md\b', line):
                # Allow only the generated AGENTS.md compatibility header to
                # reference the canonical source file by name.
                if p.name != "AGENTS.md" or not any(ctx in line for ctx in CLAUDE_MD_ALLOW_CONTEXTS):
...
def main() -> int:
    ...
    if not DRY_RUN:
        if out_dir == src_root:
            print("ERROR: refusing to write Codex build over the source root.", file=sys.stderr)
            return 1
        if out_dir.exists():
